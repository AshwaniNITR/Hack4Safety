...
export default function UnidentifiedPersonsPage() {
  ...
  const fetchAllMissingPersons = async () => {
    ...
    const cachedData = localStorage.getItem('missingPersonsData');
    const cachedTimestamp = localStorage.getItem('missingPersonsTimestamp');
    ...
    try {
      ...
      const response = await fetch('/api/fetch-found');
      ...
      if (Array.isArray(result)) {
        ...
        localStorage.setItem('missingPersonsData', JSON.stringify(result));
        localStorage.setItem('missingPersonsTimestamp', Date.now().toString());
      } else {
  ...
  const refreshData = () => {
    localStorage.removeItem('missingPersonsData');
    localStorage.removeItem('missingPersonsTimestamp');
    fetchAllMissingPersons();
  };
